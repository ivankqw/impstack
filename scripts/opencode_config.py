...
LEGACY_CLAUDE_PRESET_START = "<!-- legacy-claude-preset:start -->"
LEGACY_CLAUDE_PRESET_END = "<!-- legacy-claude-preset:end -->"
...
def finite_float(value: str) -> float:
    ...
    if not math.isfinite(number):
        raise OpenCodeConfigError(f"invalid JSON number {value}")
    return number
...
def write_mapping_if_changed(path: pathlib.Path, value: dict[str, Any]) -> bool:
    desired = (json.dumps(value, indent=2) + "\n").encode()
    try:
...
def update_config(
    ...
    path = config_path(home, environment)
    config = load_mapping(path, "OpenCode config")
    lower_priority = (
        load_mapping(path.with_suffix(".json"), "OpenCode config")
        if path.suffix == ".jsonc"
        else {}
    )
    transform(config, lower_priority)
    write_mapping_if_changed(path, config)


def add_instructions(
    config: dict[str, Any], instructions: pathlib.Path, lower_priority: dict[str, Any]
) -> None:
    existing = (
        config["instructions"]
        if "instructions" in config
        else lower_priority.get("instructions", [])
    )
...
def add_primary_config(
    ...
    add_instructions(config, instructions, lower_priority)
    add_primary_permissions(config, lower_priority)
...
def remove_legacy_claude_preset(body: str) -> str:
    starts = body.count(LEGACY_CLAUDE_PRESET_START)
    ends = body.count(LEGACY_CLAUDE_PRESET_END)
    if starts != 1 or ends != 1:
        raise OpenCodeConfigError("reviewer procedure must contain one legacy Claude preset boundary")
    start = body.index(LEGACY_CLAUDE_PRESET_START)
    end = body.index(LEGACY_CLAUDE_PRESET_END)
    if end < start:
        raise OpenCodeConfigError("reviewer legacy Claude preset boundary is out of order")
    line_end = body.find("\n", end)
    if line_end == -1:
        line_end = len(body)
    else:
        line_end += 1
    return body[:start] + body[line_end:]
...
def render_reviewer(source: pathlib.Path) -> bytes:
    ...
    procedure = remove_legacy_claude_preset("".join(lines[closing + 1 :]))
    return ("---\n" + "".join(frontmatter) + "---\n" + procedure).encode()
...
def main(argv: list[str]) -> int:
    ...
    if args.command == "instructions":
        update_config(
            args.home,
            environment,
            lambda config, lower_priority: add_primary_config(
                config, args.source, lower_priority
            ),
        )
