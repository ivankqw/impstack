...
class SkillsSyncTests(unittest.TestCase):
    ...
    def test_install_missing_rejects_an_empty_skill_after_cli_failure(self) -> None:
        _, home = self.make_home()
        repo = pathlib.Path(tempfile.mkdtemp())
        self.addCleanup(lambda: shutil.rmtree(repo))
        fakebin = pathlib.Path(tempfile.mkdtemp())
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("! failed to install skill: alpha (exit 17)", result.stderr)

    def test_install_missing_rejects_garbage_after_cli_failure(self) -> None:
        _, home = self.make_home()
        repo = pathlib.Path(tempfile.mkdtemp())
        self.addCleanup(lambda: shutil.rmtree(repo))
        fakebin = pathlib.Path(tempfile.mkdtemp())
        ...
        npx.write_text(
            "#!/usr/bin/env bash\n"
            "mkdir -p \"$HOME/.agents/skills/alpha\"\n"
            "printf '%s\\n' 'CORRUPTED PARTIAL DOWNLOAD - NOT A SKILL FILE' "
            "> \"$HOME/.agents/skills/alpha/SKILL.md\"\n"
            "echo 'network error: connection reset' >&2\n"
            "exit 17\n"
        )
        npx.chmod(0o755)
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("network error: connection reset", result.stderr)
        self.assertIn("! failed to install skill: alpha (exit 17)", result.stderr)
